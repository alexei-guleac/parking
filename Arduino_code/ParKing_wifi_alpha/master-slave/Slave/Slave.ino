...
// <-- Uncomment for using second sensor
//Sonar 2 details
...
long targetDistance = 300;                                  // target distance value when the status should be triggered, common for all sensors (can be added for each sensor)
        ...
        if (request != 0) {
            if ((request / 10U) % 10 == SLAVE_ID) {
                if ((request / 1U) % 10 == SONAR_1_ID) {
                    filteredSonarDistance_1 =
                            sonar_1.ping_median(10) / US_ROUNDTRIP_CM; // get real time distance in cm from sonar 1
                    if (!sonarInitialized_1 ||
                        ((filteredSonarDistance_1 >= targetDistance || filteredSonarDistance_1 == 0) &&
                         !isLotFreeSonar_1)
