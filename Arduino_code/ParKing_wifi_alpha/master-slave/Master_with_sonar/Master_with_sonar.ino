...
long targetDistance = 100;                                     // target distance value when the status should be triggered, common for all sensors (can be added for each sensor)
...
void (*resetFunc)(void) = 0;                                       // resetting the arduino board
...
/*########################### used for connecting sensor directly to the master #########################*/
    ...
    if (filteredSonarDistance != lastSonarDistance) {
        if (!sonarInitialized ||
            ((filteredSonarDistance >= targetDistance || filteredSonarDistance == 0) && !isLotFreeSonar)
            || (filteredSonarDistance < targetDistance && isLotFreeSonar)) {
            sonarInitialized = true;
            isLotFreeSonar = filteredSonarDistance >= targetDistance || filteredSonarDistance == 0;
//            Serial.println(isLotFreeSonar ? "SONAR_3 : FREE " + String(filteredSonarDistance) 
//                                          : "SONAR_3 : OCCUPIED " + String(filteredSonarDistance));
            client.send(isLotFreeSonar ? msg + String(13) + String("\", \"status\":\"") + status_free +
                                         String("\", \"secure_key\":\"") +
                                         security_token + String("\"}")
                                       : msg + String(13) + String("\", \"status\":\"") + status_occupied +
                                         String("\", \"secure_key\":\"") +
                                         security_token + String("\"}"));
        }
...
//          Serial.print("answer: "); Serial.println(answer);
            ...
            if (status_of_lot != "") {
                if (status_of_lot != last_known_status[i]) {
                    if (status_of_lot == "F") {
                        client.send(
                                msg + String(slave_id) + String(sensor_id) + String("\", \"status\":\"") + status_free +
                                String("\", \"secure_key\":\"") + security_token + String("\"}"));
                    } else if (status_of_lot == "O") {
                        client.send(msg + String(slave_id) + String(sensor_id) + String("\", \"status\":\"") +
                                    status_occupied + String("\", \"secure_key\":\"") + security_token + String("\"}"));
                    } else {
                        client.send(msg + String(slave_id) + String(sensor_id) + String("\", \"status\":\"") +
                                    status_unknown + String("\", \"secure_key\":\"") + security_token + String("\"}"));
                    }
                }
    ...
    for (int j = 0; j < ARRAY_SIZE(slave_ids); j++) {
        for (int i = 0; i < ARRAY_SIZE(sonar_ids); i++) {
            if (sonar_ids[i]) {
                digitalWrite(MASTER_EN, HIGH);                                // Make Enable pin high to send Data
                delay(50);                                                     // required minimum delay of 5ms
                Serial.print(slave_ids[j]);
                Serial.println(sonar_ids[i]);      // Send slave and sensor id
                Serial.flush();                                                // wait for transmission of data
                digitalWrite(MASTER_EN, LOW);                                 // Receiving mode ON
