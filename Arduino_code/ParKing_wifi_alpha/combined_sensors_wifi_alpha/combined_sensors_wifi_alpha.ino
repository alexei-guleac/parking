...
long targetDistance = 100;              // target distance value when the status should be triggered
...
const char *websockets_server_host = "68.183.149.220";  // Enter server address -- Digital Ocean droplet IP
const uint16_t websockets_server_port = 8080;           // Enter server port
const char *websockets_server_endpoint = "/demo";       // Enter server endpoint
...
void setup() {
    ...
    bool connected = client.connect(websockets_server_host, websockets_server_port, websockets_server_endpoint);
    if (connected) {
...
void loop() {
    ...
    Serial.print("filtered_1: ");
    Serial.println(filteredSonarDistance_1);
    // Serial.print("filtered_2: "); Serial.println(filteredSonarDistance_2);

    // data form sonar 1
    if (filteredSonarDistance_1 != lastSonarDistance_1) {
        if (!sonarInitialized_1 ||
            ((filteredSonarDistance_1 >= targetDistance || filteredSonarDistance_1 == 0) && !isLotFreeSonar_1)
            || (filteredSonarDistance_1 < targetDistance && isLotFreeSonar_1)) {
            lotId = 1;
            sonarInitialized_1 = true;
            isLotFreeSonar_1 = filteredSonarDistance_1 >= targetDistance || filteredSonarDistance_1 == 0;
            Serial.println(
                    isLotFreeSonar_1 ? "SONAR_1 : FREE " + String(filteredSonarDistance_1) :
                    "SONAR_1 : OCCUPIED " + String(filteredSonarDistance_1));
            client.send(isLotFreeSonar_1 ? msg + String(lotId) + String("\", \"status\":\"") + status_free +
                                           String("\", \"secure_key\":\"") +
                                           security_token + String("\"}")
                                         : msg + String(lotId) + String("\", \"status\":\"") + status_occupied +
                                           String("\", \"secure_key\":\"") +
                                           security_token + String("\"}"));
        }
    ...
    if (filteredSonarDistance_2 != lastSonarDistance_2) {
        if (!sonarInitialized_2 ||
            ((filteredSonarDistance_2 >= targetDistance || filteredSonarDistance_2 == 0) && !isLotFreeSonar_2)
            || (filteredSonarDistance_2 < targetDistance && isLotFreeSonar_2)) {
            lotId = 2;
            sonarInitialized_2 = true;
            isLotFreeSonar_2 = filteredSonarDistance_2 >= targetDistance || filteredSonarDistance_2 == 0;
            Serial.println(
                    isLotFreeSonar_2 ? "SONAR_2 : FREE " + String(filteredSonarDistance_2) : "SONAR_2 : OCCUPIED " +
                                                                                             String(filteredSonarDistance_2));
            client.send(isLotFreeSonar_2 ? msg + String(lotId) + String("\", \"status\":\"") + status_free +
                                           String("\", \"secure_key\":\"") +
                                           security_token + String("\"}")
                                         : msg + String(lotId) + String("\", \"status\":\"") + status_occupied +
                                           String("\", \"secure_key\":\"") +
...
//
//        client.send(isLotFreeLaser ? msg + String(lotId) + String("\", \"status\":\"") + status_free + String("\", \"secure_key\":\"") +
//                                         security_token + String("\"}")
//                                   : msg + String(lotId) + String("\", \"status\":\"") + status_occupied + String("\", \"secure_key\":\"") +
//                                         security_token + String("\"}"));
//    }
