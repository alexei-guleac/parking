...
/**
 * Locale support for Web API (URL route query params, cookie etc.)
 */
@Configuration
...
public class LocaleConfiguration {
    ...
    public @NotNull LocaleContextResolver localeContextResolver() {
        return new AngularCookieLocaleContextResolver();
    }

    @Bean
    public @NotNull WebFilter localeChangeFilter(@NotNull LocaleContextResolver localeContextResolver) {
        return (exchange, chain) -> {
            // Find locale change in query param. Must also look form params ?
            @Nullable String newLocale = exchange.getRequest().getQueryParams().getFirst("language");
            if (newLocale != null) {
    ...
    static class AngularCookieLocaleContextResolver implements LocaleContextResolver {
        ...
        @Override
        public void setLocaleContext(@Nonnull ServerWebExchange exchange, @Nullable LocaleContext localeContext) {
        ...
        private void addCookie(@Nonnull ServerHttpResponse response, @NotNull String cookieValue) {
            Assert.notNull(response, "ServerHttpResponse must not be null");
            @NotNull ResponseCookie cookie = ResponseCookie.from(COOKIE_NAME, cookieValue)
                .path(COOKIE_PATH)
                .build();
        ...
        private void removeCookie(@Nonnull ServerHttpResponse response) {
            Assert.notNull(response, "ServerHttpResponse must not be null");
            @NotNull ResponseCookie cookie = ResponseCookie.from(COOKIE_NAME, "")
                .path(COOKIE_PATH)
        ...
        private void parseLocaleCookieIfNecessary(@NotNull ServerWebExchange exchange) {
            if (exchange.getAttribute(LOCALE_REQUEST_ATTRIBUTE_NAME) == null) {
                // Retrieve and parse cookie value.
                @Nullable HttpCookie cookie = exchange.getRequest().getCookies().getFirst(COOKIE_NAME);
                @Nullable Locale locale = null;
                @Nullable TimeZone timeZone = null;
                if (cookie != null) {
                    @NotNull String value = cookie.getValue();

                    // Remove the double quote
                    value = StringUtils.replace(value, QUOTE, "");

                    @NotNull String localePart = value;
                    @Nullable String timeZonePart = null;
